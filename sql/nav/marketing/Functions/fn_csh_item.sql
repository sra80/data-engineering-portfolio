create or alter function marketing.fn_csh_item
    (
        @customer_id int
    )
...
return
    ...
    (
        with orders (item_id, date_start) as
            (
                select
                    a.item_id,
                    a.date_start
        ...
        , add_main (item_id, date_start) as
            (
                select
                    orders.item_id,
                    orders.date_start
        ...
        , add_end (item_id, date_start, date_end) as
            (
                select
                    x.item_id,
                    x.date_start,
                ...
                from
                    (
                        select
                            item_id,
                            date_start,
                            lead(date_start) over (partition by item_id order by date_start) date_end
                        from
                            add_main
                    ) x
            )
        , add_status (item_id, date_start, date_end, _status) as
            (
                select
                    gap.item_id,
                    gap.date_start,
                ...
                from
                    (
                        select
                            ...
                            date_start,
                            date_end,
                            datediff(day,lag(date_end) over (partition by item_id order by date_start),date_start) gap
                        from
                            add_end
                ) gap
            )
        ...
        from
            (
                select
                    ...
                    r._date,
                    r._status,
                    sum(r.is_changed) over (partition by r.item_id order by r._date) scg_id --status change group id
                from
                    (
                        select
                            q.item_id,
                            ...
                            case when
                                (
                                    q._status = lag(q._status) over (partition by q.item_id order by q._date)
                                )
                            then
                                0
                            else
