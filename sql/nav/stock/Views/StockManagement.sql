...
create or alter view [stock].[StockManagement]

as

select
    case when datediff(month,sma.key_posting_date,getdate()) <= 5 then datediff(month,sma.key_posting_date,getdate()) else datediff(year,sma.key_posting_date,getdate()) + 6 end model_partition,
    sma.company_id,
    sma.country_id,
    sma.key_posting_date,
    sma.opt_key,
    sma.is_amazon,
    sma.key_DocumentType,
    sma.key_location,
    null key_sku,
    sma.key_batch,
    sma.Quantity,
    sma.[Cost Actual],
    sma.[Cost Expected],
    sma.[Cost Posted to G_L],
    sma.[Sales Amount (Actual)],
    sma.[Discount Amount]
from
    stock.StockManagement_Archive sma
