create or alter view price_tool_feed.vw_export_pricelist
...
where
    (
        y_ep.external_id not in (select isnull(try_convert(uniqueidentifier,[External ID]),0x0) from [dbo].[UK$Sales Price Holding Table])
