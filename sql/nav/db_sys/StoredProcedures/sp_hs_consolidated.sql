create or alter procedure [db_sys].[sp_hs_consolidated]
    (
...
else if (select 1 from db_sys.hs_consolidated where table_name = @view_rebuild) = 1
...
with n as
    (
        ...
        select 
            ...
            t.column_key,
            -5 sub_key,
            concat('create or alter view hs_consolidated.[',t.table_name,']') _sql1
        from 
...
select top 1 @_sql0 = object_definition from db_sys.objects_change_control where object_id = object_id(concat('hs_consolidated.[',@table_name,']')) order by version_id desc
...
insert into db_sys.hs_consolidated (table_name) select table_name from @t where is_new = 1
