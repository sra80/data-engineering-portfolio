create or alter procedure [db_sys].[sp_fabric]
    (
...
else if (select 1 from db_sys.fabric where table_name = @view_rebuild) = 1
...
with n as
    (
        ...
        select 
            ...
            t.column_key,
            -5 sub_key,
            concat('create or alter view fabric.[',replace(t.table_name,' ','_'),']') _sql1
        from 
        ...
        select 
            ...
            concat(char(9),case when t.is_published = 0 then 'null ' else '' end,'[',t.column_name,'] [',replace(t.column_name,' ','_'),'],') _sql1
        from 
            #b7ef678c9ae54cb691f16aa338b7e908 t
        where
...
select top 1 @_sql0 = object_definition from db_sys.objects_change_control where object_id = object_id(concat('fabric.[',@table_name,']')) order by version_id desc

if left(@_sql1,5) = 'alter' if left(@_sql0,6) = 'create' set @_sql0 = concat('alter',right(@_sql0,len(@_sql0)-6))

if left(@_sql1,6) = 'create' or @_sql0 != @_sql1 print (@_sql1)
...
insert into db_sys.fabric (table_name) select table_name from @t where is_new = 1
