create or alter procedure [db_sys].[sp_set_sievo_permissions]
...
declare @user nvarchar(64) = '_sievo'
...
from
    (
        ...
        from
            @table_list t
        join
            (select left(name collate database_default,charindex('$',name)-1) _company, right(name collate database_default,len(name)-charindex('$',name)) _table, name _full from sys.tables where charindex('$',name) > 0 and create_date >= (select last_processed from db_sys.procedure_schedule where procedureName = 'db_sys.sp_set_sievo_permissions')) s
        on
            (
                t._table = s._table
            )
